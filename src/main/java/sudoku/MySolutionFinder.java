...
public class MySolutionFinder {
	...
	public int findSolution(char[] charArray) {
		initData(charArray);
		// System.out.println("");
		System.out.println(TAG + "begin to findSolution");
		if (!findSoduResult()) {
			System.out.println(TAG + "not find result findIndex" + findIndex);
			return NONE_RESULT;
		}
		hasFoundResult = true;
		System.out.println(TAG + " find result " + "   findIndex" + findIndex);
		for (int m = 80; m >= 0; m--) {
			int i = m / 9;
			int k = m % 9;
			...
			System.out.println(TAG + "find double num: i:  " + i + "   k:   " + k);
			...
			a: while (iterator.hasNext()) {
				...
				if (findSoduResult()) {
					System.out.println(
							TAG + " success  find double num: i:  " + i + "   k:   " + k + "   findIndex" + findIndex);
					return MULTI_RESULT;
				}
			}
			soduNodes[i][k].value = 0;
		...
		System.out.println(TAG + " single " + findIndex);
